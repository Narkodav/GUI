...
namespace GUI {
	...
	struct ScrollBarEventPolicy {
		using Type = ScrollBarEvent;
		...
		static void handleError(std::exception_ptr) {
			std::cerr << "[GUI] Exception in ScrollBar event callback: " << Trait<T>::s_name << std::endl;
		}
	};
}

template<>
struct GUI::ScrollBarEventPolicy::Trait<GUI::PointerTargetEvent::Scrolled> {
	using Signature = void(int scrollOffset, int deltaOffset); // In pixels
	static inline const std::string_view s_name = "Scrolled";
};

namespace GUI {

	class ScrollBar : public Rectangle {
        GUI_DECLARE_OBJECT(ScrollBar, Rectangle)
        GUI_DECLARE_OBJECT_EVENT_SYSTEM(ScrollBar, Rectangle)

    protected:
        int m_scrollOffset = 0;
        int m_maximumScrollOffset = 0;

	public:
        Button() {
            setTexture(static_cast<size_t>(DefaultTextureType::DefaultBorderEdgeTexture));;
